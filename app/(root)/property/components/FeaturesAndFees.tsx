        ...
        <div className="my-4">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {property.mainAmenities.map((item:string, index:number) => (
              <p className='border rounded-full py-2 px-3 capitalize text-base' key={index}>{item}</p>
            ))}
          </div>
        </div>
        <hr/>
        ...
        <div className="my-4">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {property.mainFees.map((item, index:number) => (
              <p className='border rounded-full py-2 px-3 capitalize text-base' key={index}>{item.name}:{' '} {nairaSign}{item.amount.toLocaleString()}.00</p>
            ))}
          </div>
        </div>
        <hr/>
