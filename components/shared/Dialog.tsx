...
const Modal = ({useCloseButton, description, children, isOpen, title, onClose, useSeparator}: modalProps) => {
    ...
    <React.Fragment>
      { isOpen &&
        <div className='fixed inset-0 overflow-hidden flex items-center justify-center outline-none focus:outline-none z-[80000] bg-neutral-800/70'>
          <div className={cn("relative sm:max-w-[480px] min-w-[390px] max-w-[410px] xl:w-2/5 my-6 mx-auto h-auto md:h-auto lg:h-auto")}>
            <div className={`translate duration-300 h-full ${showModal ? 'translate-y-0' : 'translate-y-full'} ${showModal ? 'opacity-100' : 'opacity-0'}`}>
              <div className="translate h-full lg:h-auto md:h-auto border-0 rounded shadow-lg relative flex flex-col w-full bg-card outline-none focus:outline-none">
                ...
                <div className='sm:p-5 p-4 sm:pt-0 pt-0'>
                  <div className="sm:text-base text-sm mb-4 w-full">{description}</div>
                  {useSeparator && <hr className='my-4 -mx-4 md:-mx-5'/>}
                  {children}
                </div>
              </div>
            </div>
