...
const InputWithIcon = React.forwardRef<HTMLInputElement, inputProps>(({iconClassName, className, type, disabled, icon:Icon, ...props}, ref) => {
  ...
  return (
    <div className={cn('w-full relative', className)}>
      <input type={ type === 'password' ? inputType : type } className={`cursor-pointer rounded bg-inherit w-full p-3 focus:outline-none lg:text-lg ${Icon ? 'pl-10 md:pl-12 lg:pl-12' : ''}`} disabled={disabled} {...props} ref={ref}/>
